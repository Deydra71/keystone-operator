...
import (
	...
	keystoneapi "github.com/openstack-k8s-operators/keystone-operator/api/v1beta1"
...
func (r *ApplicationCredentialReconciler) reconcileNormal(
	...
	keystoneAPI, err := keystoneapi.GetKeystoneAPI(ctx, helperObj, instance.Namespace, nil)
	if err != nil {
	...
	if instance.Status.ACID == "" {
		needsRotation = true
		logger.Info("AC does not exist, will create new one")
	} else if r.shouldRotateNow(instance) {
		needsRotation = true
		logger.Info("AC is within grace period or expired; will rotate now")
	}
	...
	if needsRotation {
		...
		userOS, userClientRes, userClientErr := keystoneapi.GetUserServiceClient(ctx, helperObj, keystoneAPI, instance.Spec.UserName)
...
func (r *ApplicationCredentialReconciler) reconcileDelete(
	...
	if instance.Status.ACID != "" {
		...
		if err == nil && keystoneAPI.IsReady() {
			...
			if userErr == nil {
				userOS, userRes, userErr2 := keystoneapi.GetUserServiceClient(ctx, helperObj, keystoneAPI, instance.Spec.UserName)
...
func (r *ApplicationCredentialReconciler) findUserIDAsAdmin(
	ctx context.Context,
	helperObj *helper.Helper,
	keystoneAPI *keystoneapi.KeystoneAPI,
	userName string,
) (string, error) {

	logger := r.GetLogger(ctx)
	adminOS, ctrlResult, err := keystoneapi.GetAdminServiceClient(ctx, helperObj, keystoneAPI)
	if err != nil {
		return "", err
	}
...
// randomSuffix generates a random string of length n using crypto/rand.
